    ...
    setDimensions(opts) {
        ...
        if (activeState) {
            for (let id in this.streams) {
                this.streams[id].offsetX = Math.floor(
                    (this.streams[id].offsetX / oldWidth) * this.width
                );
                this.streams[id].offsetY = Math.floor(
                    (this.streams[id].offsetY / oldHeight) * this.height
                );
                this.streams[id].width = Math.floor(
                    (this.streams[id].width / oldWidth) * this.width
                );
                this.streams[id].height = Math.floor(
                    (this.streams[id].height / oldHeight) * this.height
                );
            }
            this.start();
        }
