...
function App() {
    ...
    this.mediaToggles = [...document.querySelectorAll('.streamToggle')].reduce(
        (result, current) => {
...
App.prototype = {
    ...
    attachEvents: function() {
        ...
        this.presenterEl.addEventListener(
            'keyup',
            this.updatePresenterKeyUpEvent.bind(this),
            false
        );
        ...
        [...document.querySelectorAll('.streamControls label:nth-of-type(1) button')].forEach(
            btn => {
    ...
    displayStream: function(stream, value) {
        ...
        if (
            ...
            let resolutionOptions = [
                ...videoControls.querySelectorAll('label:first-of-type button')
            ];
            let doListRes = true;
            ...
            if (doListRes) {
                resolutionOptions.some((button, i) => {
                    if (
                        parseInt(resolution) < parseInt(button.value) ||
                        ...
                        resButton.addEventListener(
                            'click',
                            this.chooseResolution.bind(this),
                            false
                        );
                        return;
                    }
    ...
    chooseResolution: function(e) {
        ...
        change.then(streamObj =>
            this.displayStream(
                streamObj.stream,
                streamId === 'desktop' ? 'desktop' : 'video',
                streamId
            )
        );
    },
    ...
    setMediaLink: function(details) {
        ...
        if (anchor) {
            ...
            if (details.media.type.indexOf('video') > -1) {
                ...
                anchor.download =
                    anchor.getAttribute('data-flavor') + ' video - ' + this.title + '.webm';
                ...
                anchor.download =
                    anchor.getAttribute('data-flavor') + ' audio - ' + this.title + '.webm';
    ...
    if (!navigator.mediaDevices || !('getDisplayMedia' in navigator.mediaDevices)) {
        ...
        window.addEventListener('message', e => {
            if (
                e.data.type &&
                e.data.type == 'SS_PING' &&
                document.getElementById('appInstalled')
            ) {
                clearTimeout(delay);
            }
        });
